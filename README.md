...
XJB_TRADE (TEA) 1.0.0　2026-08-03 14:10 | 买入窗口(14:00-14:45) | 买入窗口=开
  ...
  10. 持仓 / 资金
   m. 展开全部 22 项
   q. 退出　│　1-22 任意编号都可直接输入
```

| 输入 | 作用 |
|---|---|
| `1`–`22` | 直接执行对应功能。**编号永不变**，没列出来的也能输 |
| `m` | 展开全部 22 项（按道法 / 准入 / 计划 / 持仓 / 复盘 / 工具 分组） |
| `c` | 收起，回到四条建议 |
| 直接回车 | 重印菜单（不算选错） |
| `q` | 退出 |

为什么默认不全列：同一时刻真正有意义的操作从来不超过四个。非交易日只能复盘和演练，买入窗口外再怎么评估也会被门禁拦回——把全部选项平铺在眼前只会让人每次重新读一遍。挂着过期旧计划（还有未执行标的、执行日已不是今天）时，建议里会自动顶出 `22. 清除过期计划`。
