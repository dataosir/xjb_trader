...
- **CDN 节点按个坑时，坏节点会拖垮整次取数**：实测 `push2` / `push2his` 某些节点直接 `RemoteDisconnected`，而 `push2delay` 这类节点是通的。旧轮换每次 `random.choice`，一次操作里多个请求各自随机挑节点，可能三次重试全撑在同一个坏节点上，而且不记事——上一个请求刚确认 push2 死了，下一个又去撞（这就是为何同一节点池下板块成功而涨跌比失败）。现在：同一次 `get_json` 的多次重试走不同节点；连接失败的节点记入会话黑名单，后续请求先活后死排序（成功则自动复活，整池全坏仍会把每个节点都试一遍）。kline 节点池从 2 个扩到 4 个（`push2his` / `1` / `2` / `7`），`retries` 3 → 4 以盖住最大节点池。实测：在 `push2` / `push2his` 均被封的环境下，默认配置无需手工指定主机即可拿全指数 / 涨跌比 / 板块 / 涨停池
...
- 自测 97 → **158 项**。新增：用东财真实 `fltt=2` payload 验证行情解析量级（含尺度无关的 `(price/pre_close-1)*100 == chg_pct` 自洽断言）、模拟 clist 硬限行为验证精确涨跌家数与板块成分不被截断、涨停池按交易日回退、坏令牌不被当成「涨停 0 家」且不触发冰点降仓、CDN 节点故障转移（坏节点不拖垮取数、会话内记住坏节点、整池全坏仍逐个试）、菜单分组与 MENU 完全对齐、各时段建议 1-4 条且均合法
