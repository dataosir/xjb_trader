...
已落地（代码）：…**F16 观察池盘中邮件提醒**（…**邮件含建议挂单价/止损/止盈/计划止盈 T+1~T+3**；**种子可买/观察/前夕主列表 + 候选明细同步展示挂单价/计划止盈路径**；**挂单价改乖离自适应纪律回踩（非 MA20 锚定），回踩轨止损/止盈按挂单价重算**；**控制台/SEED 核心数据高亮**；**F01 市场天气屏语义色高亮（热点/情绪/涨跌比/连板涨停等）**）、**F17 每周选股周报邮件**…
