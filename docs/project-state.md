...
**巩固可买硬闸（突破+过热禁买、`winrate_score`、板块一致性）与样本落盘 → 用真实回填验证口径 → 再开阶段 2 归因。**

已落地（代码）：突破/过热禁买、`winrate_score` 可买门槛、入选板块一致性、低吸阶段 1 只攒不买、因子/标签落盘修复；自动轻量回填（种子收尾一行摘要）+ 菜单合并为约 10 项。

## 进行中

| ID | 事项 | 说明 |
|---|---|---|
| B-P0-01 | 每日 `seed-plan`（含自动回填）+ 适时全量 `review` | 运营纪律；轻量回填不替代观察池复核 |
| B-P0-02 / 07 | 因子与 `lowbuy` 样本积累 | 目标低吸回填 ≥30 再谈验证 |
| B-P1-01 | rule vs winrate 影子对照 | 1–2 周，不写计划；入口在复盘工具▸ |
...
- 自动下单 / 拆除纪律  
- 「三日必赚」承诺（系统只提高期望，不保证收益）

详见 [`prd/05-roadmap-backlog.md`](prd/05-roadmap-backlog.md)。

## 下一步计划

1. 按新口径（突破+过热禁买）持续跑日循环；今日已写的过热可买计划建议人工作废后再看新闸门。  
