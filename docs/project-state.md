...
已落地（代码）：突破/过热禁买、`winrate_score`、板块一致性、**自动轻量回填（默认后台异步）**、候选否决原因必展示、**样本缺口看板**、**`shadow_tag` 落盘对照**、**大盘指数超时修复 + MA20 跨源补全 + `tea.data` 运行日志**、**共振分/行情关键节点 `tea.log` 追溯 + launchd 直调 python 修复**、**东财 K 线会话熔断（push2his 封禁时直切腾讯）+ 缺口横幅误报修复**。
