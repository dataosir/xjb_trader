...
- **单行摘要**：种子扫描头 `format_sentiment_summary` 与天气屏共用着色规则；TRADE/SEED Markdown 报告热点行加粗。  
- **自测**：`check_sentiment` 增补天气 ANSI / 热点 / 情绪分着色断言。

### 体验（2026-09-10：挂单价改纪律回踩 + 止损按挂单价重算）

- **挂单价算法**：观察/前夕/候选不再锚 MA20；改为**乖离自适应纪律回踩**（≤5%→2%、5–10%→3%、>10%→5%），高乖离时 MA20 仅作参考标注。  
- **止损/止盈**：回踩轨以**挂单价为入场**重算 stop/target（不再把挂单价抬到扫描价止损）；可买（现价）仍沿用预审 ATR 结构。  
- **自测**：605006/600979/邮件/可买路径断言同步；740/740 通过。

### 体验（2026-09-10：F16 邮件含建议挂单价）

- **建议挂单价**：`watch_pool.suggest_limit_price` — 回踩优先贴近 MA20 支撑，按 A 股最小价位向下取整，不高于现价。  
- **邮件正文**：观察池提醒新增「建议挂单价 / 止损 / 止盈」行，便于盘中直接填券商限价单。  
- **种子候选明细**：控制台与 SEED 报告候选区同步展示挂单价（复用 `attach_order_hint`）。  
- **待启动观察主列表**：`[观察]` 行同步展示挂单价（`attach_output_order_hints`）。  
- **核心数据高亮**：种子扫描控制台对现价/涨幅/共振/止损止盈/R:R/挂单价等走语义色 ANSI；SEED 报告表格与挂单价行用 Markdown 加粗。  
- **低吸观察 + 计划止盈路径**：前夕主列表与候选明细同步展示挂单价；新增 T+1/T+2/T+3 分段目标（挂单价→止盈价线性拆分，复盘对照，非概率）。  
- **可买主列表计划止盈**：`[买入]` 行单独展示挂单价与 T+1/T+2/T+3 路径（控制台高亮 + SEED 加粗）。  
- **邮件计划止盈**：F16 提醒正文改为多行「计划止盈路径」；挂单价/止损/止盈/T+N 用【】强调（纯文本可读）。  
- **工具**：`utils.price_tick` / `round_price_down` / `round_price_up`。  
- **自测**：挂单价取整 + 邮件正文 + 候选明细 + 观察/可买主列表 + 高亮断言。
...
### 功能（2026-09-09：F16 邮箱引导配置）

- **新增** `tea setup-email`：163 SMTP 引导向导（授权码步骤说明、发件/收件、测试邮件）；`--test` 仅测当前配置。  
- **菜单**：配置与维护 → 邮件提醒邮箱配置。  
- **模块**：`tea/config/email_setup.py`；`IO.ask_secret` 支持授权码不回显输入。  
- **自测**：向导落盘、测试邮件 mock、取消不写盘。
...
- **`review` 打印低吸样本进度**（`followthrough.lowbuy_sample_stats`）：累计条数、已回填条数、
  胜率，目标 ≥30 条进入低吸验证。
- **标签区分**：`可买` 更名为 `可买（追高）`、`低吸观察` 更名为 `低吸观察（启动前夕）`，
  候选明细与报告一眼分清「追高」和「低吸」。

### 策略（2026-08-25 低吸阶段 1：启动前夕板块池 + 落盘标签）

> 低吸（启动前夕）技术方案的阶段 1：只积累样本、不买入。追涨吃 TOP1-3 已涨停板块
> 的鱼尾，低吸吃排名 3~10、刚开始升温（涨幅 2~4%、涨停≤1）板块的鱼身。

- **低吸板块池 `screener.lowbuy_sector_pool`**：`seed.lowbuy_rank_min/max=3/10`、
  `lowbuy_chg_min/max=2/4`、`lowbuy_limit_up_max=1`。
- **前夕观察改用低吸板块池**：`seed_scan` 不再对 TOP3 扫前夕，改扫低吸板块池。
- **落盘标签**：低吸候选 `lowbuy=True` 进 seed_records，配合已有的 bias_ma20/vol_ratio/
  intraday/sector_rank 等字段，可单独归因。
- **文案**：报告「前夕观察 · 永不写计划」→「低吸观察 · 暂不写计划，积累样本」。
