# 文档归档（archive）

本目录存放**有价值但不作为现行主文档**的历史笔记。  
现行需求与 backlog 以 [`../prd/`](../prd/README.md) 为准；细节实验仍可从本目录引用。  
全库清单见 [`../INDEX.md`](../INDEX.md)。

| 文件 | 说明 |
|---|---|
| `WINRATE_ROADMAP_2026-08-24.md` | 胜率提升三阶段路线图 |
| `WINRATE_PRIOR_PLAN_2026-08-24.md` | 阶段 B 胜率先验（HOLD） |
| `LOWBUY_PLAN_2026-08-25.md` | 低吸（启动前夕）三阶段方案 |
| `STRATEGY_ADJUSTMENT_2026-08-18.md` | 早期过滤器死锁修复 |
| `CODE_REVIEW_2026-08-18.md` | 代码评审纪要 |

**规则**：不要把新的主 PRD/tech 写进 archive；归档文件默认只读，修订结论应回写到 `prd/` / `CHANGELOG.md` / `project-state.md`。
