...
- [ ] 候选明细（有预审行情者）展示建议挂单价 / 止损 / 止盈（与 F16 同一算法；**乖离自适应纪律回踩**，非 MA20 锚定；止损/止盈按挂单价重算）  
- [ ] 待启动观察主列表（`[观察]` 行）同步展示建议挂单价 / 止损 / 止盈  
- [ ] 控制台现价/涨幅/挂单价等核心数据语义色高亮；SEED 报告对应字段 Markdown 加粗  
- [ ] 可买主列表（`[买入]` 行）单独展示建议挂单价 / 止损 / 止盈，并附 T+1/T+2/T+3 计划止盈路径（复盘对照，非概率）  
- [ ] 低吸观察主列表 + 候选明细展示建议挂单价 / 止损 / 止盈，并附 T+1/T+2/T+3 计划止盈路径  
