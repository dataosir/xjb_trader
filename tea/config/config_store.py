...
DEFAULTS: Dict[str, Any] = {
    ...
    # ---------------------------------------------------------- 胜率选股（数据型通道）
    # 与 9 分共振（纪律型 rule）并行：winrate_score 按实证胜率因素加权（见
    # preflight.winrate_score），达标即胜率可买；只落盘 mode=winrate，不写计划不买入。
    "winrate": {
        "buyable_threshold": 3,
    },
    # ---------------------------------------------------------- 种子扫描细则
    ...
    "followthrough": {
        ...
        "auto_backfill_async": True,         # True 则自动回填走后台线程，不阻塞种子/菜单
        # 样本缺口看板 / 影子桶（只对照不驱动计划）
