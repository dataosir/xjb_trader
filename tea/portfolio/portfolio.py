...
def add_manual_position(code: str, name: str, shares: int, entry: float,
                        ...
                        opened_date: Optional[str] = None) -> dict:
    """手动录入 / 更新持仓（代码维度幂等，供已在券商实盘买入的票补登）。

    与 open_position 的区别：股数/成本由用户直接给定，没有 sizing 计算，
    也没有 9 分共振/身份快照；阶段直接记为满仓，来源标 manual 便于区分。

    幂等：同一代码重复录入不新增，而是覆盖更新股数/成本/名称/止损止盈
    （保留 id 与建仓时间），返回 {"pos": ..., "updated": bool}。
    """
    cfg = cfg or load_config()
    code = utils.norm_code(code)
    shares = int(shares)
    ...
    for p in st["positions"]:
        if p.get("code") == code:
            ...
            save_state(st, cfg)
            return {"pos": p, "updated": True}
    pos = {
        "id": f"{code}-{utils.stamp()}", "code": code,
        "opened_at": utils.now().strftime("%Y-%m-%d %H:%M:%S"),
